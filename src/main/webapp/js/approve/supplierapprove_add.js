/**
 * Created by infoyb on 2017.10.24.
 */
...
$(function() {
    ...
    //所属地区模糊查询
    // $("input[name=areaName]").autocomplete({
    //     source:function( request, response ) {
    //         $.ajax({
    //             url : "/approveBase/selectByAreaName",
    //             type : "post",
    //             dataType : "json",
    ...
    //     minLength: 1,  // 输入框字符个等于2时开始查询
    //     focus: function( event, ui ) {
    //         $("input[name=areaName]").val( ui.item.label );
    //         return false;
    //     },
    //     select: function( event, ui ) {// 选中某项时执行的操作
    //         $("input[name=areaName]").val( ui.item.label );
    //         $("input[name=area]").val( ui.item.value );
    //         return false;
    //     }
    // });
    ...
    uploader.on('uploadSuccess', function(file, response) {
        ...
        //console.info(file);
        console.info("已上传。。。");
        $("#thelist").append("<p class=\"file-other-title\" id="+file.id+" style=\"text-align: left\"><span>"+file.name+"</span><button class=\"btn red\" onclick=\"deleteDoc('"+file.id+"','"+response.msg[0].docUrl+"')\">删除</button></p>");
 ...
 * @param flag
 */
function addSupplierApproveBase (flag) {
    var isValid = false;
    if (flag == 1) {
